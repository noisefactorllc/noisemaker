...
test('renderCube exports 6 distinct cube faces (readback works on this backend)', async ({ page }) => {
  ...
  expect(result.seam.worstMean, JSON.stringify(result.seam)).toBeLessThan(0.0015)
  expect(result.seam.worstMax, JSON.stringify(result.seam)).toBeLessThan(0.004)
})

test('renderCubemap3d exports seam-continuous lit cube faces', async ({ page }) => {
  const projectName = test.info().project.name || ''
  const useWGSL = process.env.BACKEND === 'wgsl' || projectName.includes('webgpu')
  ...
  const DSL = [
    ...
    'noise3d(volumeSize: x64)', '  .renderCubemap3d()', '  .write(o0)', '',
    'render(o0)',
  ].join('\n')
  ...
  const result = await page.evaluate(async (dsl) => {
    ...
    const r = window.__noisemakerCanvasRenderer
    await r.loadEffects(['synth3d/noise3d', 'render/renderCubemap3d'])
    await r.compile(dsl)
    const p = r.pipeline
    if (r.stop) r.stop()
    ...
    for (const face of faces) {
      ...
      for (let i = 0; i < data.length; i += 16) {
        if (data[i] > 0 || data[i + 1] > 0 || data[i + 2] > 0) { nonzeroRgb = true; break }
      }
      if (nonzeroRgb) break
    }
    return {
  ...
  expect(result.nonzeroRgb, 'lit cubemap emits nonblack color').toBe(true)
  expect(result.distinct, 'all six faces distinct (cubeBasis varies per face)').toBe(6)
  expect(result.seam.worstMean, JSON.stringify(result.seam)).toBeLessThan(0.0015)
  expect(result.seam.worstMax, JSON.stringify(result.seam)).toBeLessThan(0.004)
})
