...
```
effectName(paramName: 0.8).write(o0)
```

This section is optional but recommended for non-trivial effects.
...
1. **H1 title required** — Always start with `# Effect Name`
...
5. **Range format** — Use `min–max` (en-dash), enum values, or `—` for N/A
6. **Code blocks** — Use triple backticks, no language specifier needed
7. **Section order** — Title → Description → Parameters → Usage → Notes
