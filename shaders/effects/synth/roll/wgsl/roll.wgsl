...
fn main(@builtin(position) pos: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let scrollUv = vec2<f32>(uv.x - scrollAmount, uv.y);
    var prev = vec4<f32>(0.0);
    if (scrollUv.x >= 0.0) {
        prev = textureSample(feedbackTex, feedbackSampler, scrollUv) * 0.997;
    }
    ...
    // Sample note grid with spread for visibility
    var maxVel = 0.0;
    let lanePixels = u.resolution.y / 16.0;
    let keysPerPixel = f32(keyRange) / lanePixels;
    let spread = max(1, i32(ceil(keysPerPixel)));
