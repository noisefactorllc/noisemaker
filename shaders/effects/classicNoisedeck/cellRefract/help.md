# cellRefract

Cell-based refraction

## Parameters

| Parameter | Type | Default | Range | Description |
|-----------|------|---------|-------|-------------|
| refractAmt | float | 23 | 0-100 | Refract |
| refractDir | float | 0 | 0-360 | Refract dir |
| wrap | int | mirror | mirror/repeat | Wrap |
| speed | int | 1 | 0-5 | Speed |
| shape | int | diamond | circle/diamond/hexagon/octagon/square/triangle | Shape |
| scale | float | 50 | 1-100 | Scale |
| cellScale | float | 75 | 1-100 | Cell scale |
| cellSmooth | float | 0 | 0-100 | Cell smooth |
| cellVariation | float | 0 | 0-100 | Cell variation |
| seed | int | 1 | 1-100 | Seed |
| effect | int | none | none/blur/derivatives/derivDivide/edge/emboss/litEdge/outline/pixels/posterize/shadow/sharpen/sobel | Effect |
| effectWidth | int | 0 | 0-10 | Effect width |


