...
@group(0) @binding(3) var<uniform> shadowMaskSource: i32;
@group(0) @binding(4) var<uniform> shadowSourceChannel: i32;
@group(0) @binding(5) var<uniform> shadowThreshold: f32;
@group(0) @binding(6) var<uniform> shadowColor: vec3<f32>;
@group(0) @binding(7) var<uniform> shadowOffsetX: f32;
@group(0) @binding(8) var<uniform> shadowOffsetY: f32;
@group(0) @binding(9) var<uniform> shadowBlur: f32;
@group(0) @binding(10) var<uniform> shadowSpread: f32;
@group(0) @binding(11) var<uniform> shadowWrap: i32;
...
fn main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let maskUV = uv - vec2<f32>(shadowOffsetX, shadowOffsetY) * 0.1;

    // Gaussian blur of thresholded mask
    var shadowMask: f32 = 0.0;
    var totalWeight: f32 = 0.0;

    let sigma = max(shadowBlur, 0.001);
    let sigma2 = 2.0 * sigma * sigma;

    for (var x: i32 = -5; x <= 5; x = x + 1) {
        for (var y: i32 = -5; y <= 5; y = y + 1) {
            let offset = vec2<f32>(f32(x), f32(y)) * shadowBlur / dims;
    ...
    let withShadow = mix(baseColor.rgb, shadowColor, shadowMask);
    ...
    let fgMask = step(shadowThreshold, getChannel(fgSample, shadowSourceChannel));
